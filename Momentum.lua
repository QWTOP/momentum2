...
indicatorBar.Size = UDim2.new(1, 0, 0, 24)
indicatorBar.Position = UDim2.new(0, 0, 1, -24)
indicatorBar.BackgroundColor3 = BG_MID
indicatorBar.BorderSizePixel = 0
indicatorBar.Parent = visualTab
addCorner(indicatorBar, 0)

local indLayout = Instance.new("UIListLayout")
indLayout.FillDirection = Enum.FillDirection.Horizontal
indLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
indLayout.VerticalAlignment = Enum.VerticalAlignment.Center
indLayout.Padding = UDim.new(0, 6)
indLayout.Parent = indicatorBar

local indPadding = Instance.new("UIPadding")
indPadding.PaddingLeft = UDim.new(0, 8)
indPadding.PaddingRight = UDim.new(0, 8)
indPadding.Parent = indicatorBar

local indicators = {}

local function makeIndicator(name, tag)
    local lab = Instance.new("TextLabel")
    lab.Size = UDim2.new(0, 0, 0, 18)
    lab.AutomaticSize = Enum.AutomaticSize.X
    lab.BackgroundTransparency = 0.7
    lab.BackgroundColor3 = BG_LIGHT
    lab.Text = " " .. tag .. " "
    lab.Font = Enum.Font.GothamBold
    lab.TextSize = 10
    lab.TextColor3 = TXT_DIM
    lab.Parent = indicatorBar
    addCorner(lab, 3)
    indicators[name] = lab
    return lab
end
...
local function updateIndicators()
    ...
    for name, lab in pairs(indicators) do
        if states[name] then
            lab.BackgroundColor3 = ACCENT
            lab.TextColor3 = TXT_BRIGHT
